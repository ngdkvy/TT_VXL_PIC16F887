...
unsigned int8 dv_giay_lcd, ch_giay_lcd;
...
void giai_ma_led_quet_giay() 
{         
    led_7dq[3] = ma7doan [giay %10];                     
    led_7dq[4] = ma7doan [giay/10];  
    if(led_7dq[4] == 0xc0) led_7dq[4] = 0xff;
...
{ 
   ...
   ch_giay_lcd = giay/10%10 + 0x30;
   if(ch_giay_lcd == 0x30) ch_giay_lcd = 0x20; //32 = ' ' : khoang trang
} 

void HT_LCD_GIAY() 
{ 
    lcd_goto_xy(2,0);      //hang 2, cot 0
    lcd_data("DEM GIAY");
    
    lcd_goto_xy(3,12);   //xac dinh vi tri - hang 3, cot 18
    delay_us(20); 
    lcd_data(ch_giay_lcd); 
